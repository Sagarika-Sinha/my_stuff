...
int main() {
    ...
    std::vector<int> distances = {4, 16, 64};
    std::vector<const char*> hints = {"T0", "NTA"};

    const int REPS = 5;

    printf("size,width,distance,hint,time_naive_s,time_optimized_s,speedup\n");
    fflush(stdout);
    ...
    for (int size : sizes) {
        ...
        for (auto width : widths) {
            ...
            for (auto hint : hints) {
                setenv("PREFETCH_HINT", hint, 1);
                for (int dist : distances) {
                    setenv("PREFETCH_DISTANCE", std::to_string(dist).c_str(), 1);

                    fprintf(stderr, "size=%d width=%s hint=%s dist=%d : running optimized...\n",
                            size, width, hint, dist);
                    double t_opt = time_median([&]() {
                        matmul_optimized(A.data(), B.data(), C.data(), M, N, K, lda, ldb, ldc);
                    }, REPS);

                    double speedup = t_naive / t_opt;

                    printf("%d,%s,%d,%s,%.6f,%.6f,%.4f\n",
                           size, width, dist, hint, t_naive, t_opt, speedup);
                    fflush(stdout);
                }
            }
        }
